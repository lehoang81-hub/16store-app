...
function CountdownBadge({ until }: { until: string }) {
  ...
  return <span className={`font-['Space_Mono'] text-sm ${urgent ? 'text-rust' : 'text-hazard'}`}>{display}</span>
}

function OrderCard({ order }: { order: Order }) {
  const statusCfg = STATUS_CONFIG[order.status] ?? { label: order.status, color: 'text-bone-2', bg: 'border-line' }
  ...
  return (
    <div className={`border p-5 ${statusCfg.bg}`}>
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-3 mb-2 flex-wrap">
            <span className="font-mono text-[10px] tracking-[0.22em] uppercase text-rust">LOT {order.lot_id}</span>
            <span className={`font-mono text-[10px] tracking-[0.14em] uppercase ${statusCfg.color}`}>{statusCfg.label}</span>
          </div>

          {order.posts && (
            <div className="mb-3">
              <p className="font-display text-lg uppercase leading-tight">{order.posts.brand} {order.posts.model}</p>
              <p className="font-serif italic text-rust">&ldquo;{order.posts.colorway}&rdquo;</p>
              <p className="font-mono text-[10px] text-concrete tracking-[0.14em] uppercase mt-0.5">US {order.posts.size_us}</p>
            </div>
          )}
          ...
          <div className="flex items-center gap-5 flex-wrap">
            <div>
              ...
              <p className="font-['Space_Mono'] text-hazard font-bold">{formatVND(order.amount_vnd)} ₫</p>
            </div>
            <div>
              <p className="font-mono text-[10px] text-concrete tracking-[0.14em] uppercase mb-0.5">Mã CK</p>
              <p className="font-['Space_Mono'] text-bone text-sm font-bold">{order.vietqr_ref}</p>
            </div>
            <div>
              <p className="font-mono text-[10px] text-concrete tracking-[0.14em] uppercase mb-0.5">Ngày đặt</p>
              <p className="font-mono text-[11px] text-bone-2">{new Date(order.created_at).toLocaleDateString('vi-VN')}</p>
            </div>
          </div>
        ...
        <div className="flex flex-col gap-2 items-end">
          {order.status === 'reserved' && (
            <Link href={`/checkout/${order.id}`} className="bg-rust text-ink px-4 py-2.5 font-mono text-[11px] font-bold tracking-[0.18em] uppercase hover:bg-bone transition-colors whitespace-nowrap">
              Thanh toán →
            </Link>
          )}
          {order.status === 'completed' && order.passport && (
            <Link href={`/passport/${order.passport.qr_code}`} className="bg-rust text-ink px-4 py-2.5 font-mono text-[11px] font-bold tracking-[0.18em] uppercase hover:bg-bone transition-colors whitespace-nowrap">
              Xem hộ chiếu →
            </Link>
          )}
          {order.status === 'completed' && (
            <button
              ...
              className="font-mono text-[10px] tracking-[0.14em] uppercase text-hazard hover:text-bone transition-colors"
            >
              {showReview ? 'Ẩn đánh giá ▲' : '⭐ Đánh giá →'}
            </button>
          )}
          <Link href={`/lot/${order.lot_id}`} className="font-mono text-[10px] tracking-[0.14em] uppercase text-concrete hover:text-bone-2 transition-colors">
            Xem lot →
          </Link>
        </div>
      </div>
      ...
      {/* Review Form */}
      {showReview && order.status === 'completed' && (
        <div className="mt-4 border-t border-line pt-4">
          <ReviewForm
            orderId={order.id}
            sellerId={order.seller?.id ?? ''}
            sellerHandle={order.seller?.handle ?? order.lot_id}
  ...
  if (loading) {
    return (
      <div className="text-center py-20">
        <p className="font-mono text-[10px] tracking-[0.22em] uppercase text-concrete animate-pulse">Đang tải...</p>
      </div>
    )
  }
  ...
  if (orders.length === 0) {
    return (
      <div className="border border-dashed border-line p-12 text-center">
        ...
        <Link href="/#floor" className="bg-rust text-ink px-6 py-3 font-mono text-[11px] font-bold tracking-[0.18em] uppercase hover:bg-bone transition-colors">
          Xem Floor →
        </Link>
      </div>
    )
    ...
    <div className="space-y-10">
      ...
      {completedOrders.length > 0 && (
        <section>
          ...
          <div className="space-y-3">{completedOrders.map((o) => <OrderCard key={o.id} order={o} />)}</div>
        </section>
      )}
      {cancelledOrders.length > 0 && (
        <section>
          ...
          <div className="space-y-3 opacity-60">{cancelledOrders.map((o) => <OrderCard key={o.id} order={o} />)}</div>
        </section>
      )}
    </div>
  )
