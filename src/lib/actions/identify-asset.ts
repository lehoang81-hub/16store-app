...
export async function identifyAsset(
  ...
  try {
    ...
    // Get reward_points từ users table
    ...
    const { data: userFull } = await supabase
      ...
      .single();
    const currentHlr = userFull?.reward_points ?? 0;

    // ── Duplicate check (24h cooldown) ──────────────────────
    ...
    const coverImageUrl = uploadedUrls.hero ?? allImageUrls[0] ?? null;

    const imageAngles: Record<string, string> = {};
    for (const [k, url] of Object.entries(uploadedUrls)) {
      if (url) imageAngles[k] = url;
    }
    ...
    const { data: passport, error: mintError } = await supabase
      ...
      .insert({
        ...
        image_ai_verified: false, // Enabled in Phase 2 with Gemini
...
Định danh bởi @${userProfile.handle ?? 'unknown'} · THE FIRST CLAIMANT.`,
    ...
    await supabase
      .from('users')
      .update({ reward_points: currentHlr + HLR_REWARD })
      .eq('user_id', userProfile.id);
